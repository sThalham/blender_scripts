...
for num_set in np.arange(total_set):
    ...
    bpy.data.objects["Plane.Ground"].dimensions=(random()*0.5+0.5, random()*0.5+0.5, 0.05)
    ...
    for i in np.arange(num_object):
        ...
        draw_x = uniform(-0.2, 0.2)
        draw_y = uniform(-0.2, 0.2)
        anchor_pose[i,0] = draw_x
        anchor_pose[i,1] = draw_y
    ...
    for i in np.arange(num_back2):
        ...
        obj_object.name = 'linemod' + obj_object.name
        draw = uniform(0.3, 0.6)
        ang = uniform(0, 2*pi) -pi
    ...
    for iii in np.arange(pair_set):
        ...
        for obj in scene.objects:
            ...
            if obj.name == 'InvisibleCube':
                obj_object.rotation_euler.x=radians(random()*55.0+10.0) #0~90
                obj_object.rotation_euler.y=radians(random()*30.0-15.0) #-45-45
                obj_object.rotation_euler.z=radians(random()*360.0)
                print(obj_object.rotation_euler)

            if obj.type == 'CAMERA' and  obj.name=='cam_L':
                obj_object = bpy.data.objects[obj.name]
                obj_object.location.z = random()*0.1+0.95   # YCBV BOP: 0.677-1.219
        ...
        lamp_position = ((random()*2.8-1.4), (random()*2.8-1.4), (random()*1.0+2.0))
        ...
        lamp_object.data.node_tree.nodes['Emission'].inputs['Strength'].default_value = 200.0 + random()*100.0
        ...
        ambient_light.data.node_tree.nodes['Emission'].inputs['Strength'].default_value = 200.0 + random()*100.0
