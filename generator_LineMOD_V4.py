...
sample_dir = '/home/stefan/data/rendered_data/linemod_rgbd_V4' #directory for temporary files (cam_L, cam_R, masks..~)
target_dir = '/home/stefan/data/rendered_data/linemod_rgbd_V4/patches'
index=0
isfile=True
while isfile:
...
back_img_path = '/home/stefan/data/coco_test2017'
...
for num_set in np.arange(total_set):
    ...
    for obj in scene.objects:
        if obj.type == 'MESH':
            if obj.name[0:8] == 'template':
                ...
                obj.select = False
            elif obj.name == 'Speaker1':
                obj.select = False
            elif obj.name[:4] == 'Area':
                obj.select = False
            else:
                obj.select = True
        if obj.name[0:6] == 'light':
            obj.select = True
    ...
    glo_shift_x = random()*0.2 - 0.1
    glo_shift_y = random()*0.2 - 0.1
    ...
    for i in np.arange(num_object):
        ...
        # max dimension
        max_x = obj_object.dimensions[0]
        max_y = obj_object.dimensions[1]
        max_d = (max_x + max_y)*0.5
        #max_d = 0.0
        #if max_x > max_y:
        #    max_d = max_x
        #else:
    ...
    #     6-------------4
        ...
        if asc_i == 0:
            pose_x = glo_shift_x
            pose_y = glo_shift_y
            middle_dia = 0.5 * max_d
        elif asc_i == 1: # + y 0 x
            off_x = random()*0.1 - 0.05
            off_y = random()*0.05
            pose_x = glo_shift_x + off_x
            pose_y = glo_shift_y + (max_d * 0.5) + middle_dia + off_y
            plux_back += off_x
            minx_back += off_x
            dia_1 = max_d
        elif asc_i == 2:
            off_x = random()*0.05
            off_y = random()*0.1 - 0.05
            pose_x = glo_shift_x + (max_d * 0.5) + (middle_dia ) + off_x
            pose_y = glo_shift_y + off_y
            pluy_rig += off_y
            miny_rig += off_y
            dia_2 = max_d
        elif asc_i == 3:
            ...
            off_y = random()*0.05
            pose_x = glo_shift_x + (max_d * 0.5) + (middle_dia) + off_x
            pose_y = glo_shift_y -(max_d * 0.5) - (dia_2 * 0.5) - off_y
        elif asc_i == 5:
            off_x = random()*0.05
            off_y = random()*0.05
            pose_x = glo_shift_x + (max_d * 0.5) + (dia_1 * 0.5) + off_x
            pose_y = glo_shift_y + (max_d * 0.5) + (dia_2 * 0.5) + off_y
            pluy_lef += off_y
            miny_lef += off_y
        elif asc_i == 6:
            off_x = random()*0.05
            off_y = random()*0.05
            pose_x = glo_shift_x - (max_d * 0.5) - (middle_dia * 0.5) - off_x
            pose_y = glo_shift_y - (max_d * 0.5) - (dia_3 * 0.5) - off_y
        elif asc_i == 7:
            off_x = random()*0.05
            off_y = random()*0.05
            pose_x = glo_shift_x - (max_d * 0.5) - (dia_1 * 0.5) - off_x
            ...
            #pluy_lef += off_y
            #miny_lef += off_y
        print(asc_i, pose_x, pose_y)
        asc_i += 1
            
        anchor_pose[i,0] = pose_x
        anchor_pose[i,1] = pose_y
        anchor_pose[i,2] = 0.14
        anchor_pose[i,3] = 0.0
        anchor_pose[i,4] = 0.0
    ...
    for i in np.arange(Back_object):
        ...
        sector = randint(1,3)
        if sector == 1:
            pose_x = glo_shift_x -0.35 - random()*0.4
            pose_y = glo_shift_y -0.2 + random()*0.8
        elif sector == 2:
            pose_x = glo_shift_x + 0.35 + random()*0.4
            pose_y = glo_shift_y - 0.2 + random()*0.7
        elif sector == 3:
            pose_x = glo_shift_x + 0.75 - random()*1.5
            pose_y = glo_shift_y + 0.35 + random()*0.25
        print(pose_x, pose_y)

        anchor_pose[i+num_object,0] = pose_x
        anchor_pose[i+num_object,1] = pose_y
        anchor_pose[i+num_object,2] = 0.1 + random()*0.2
    ...
    for i in np.arange(num_back2):
        ...
        anchor_pose[i+num_object+Back_object,0] = pose_x
        anchor_pose[i+num_object+Back_object,1] = pose_y
        anchor_pose[i+num_object+Back_object,2] = 0.1 + random()*0.2
    ...
    for obj in scene.objects:
        ...
        if obj.name in ['Basket', 'Imac', 'Keyboard', 'Lamp1', 'Laptop', 'Monitor2', 'Mouse', 'Pot', 'Potplant', 'Speaker1']:
            obj_object= bpy.data.objects[obj.name]
            img_choice = Rchoice(all_backgrounds)
            img_path = os.path.join(back_img_path, img_choice)
            mat_now = obj_object.active_material
    ...
    for iii in np.arange(pair_set):
        ...
        for obj in scene.objects:
            ...
            if obj.name == 'InvisibleCube':
                obj_object.rotation_euler.x=radians(random()*82.5+5) #0~90
                obj_object.rotation_euler.y=radians(random()*90.0-45.0) #-45-45
                obj_object.rotation_euler.z=radians(random()*90.0-45.0)

            if obj.type == 'CAMERA' and  obj.name=='cam_L':
                obj_object = bpy.data.objects[obj.name]
                #obj_object.location.z = random()*0.3+0.75  #1.0-2.5
                #obj_object.location.z = random()*0.45+0.7
                obj_object.location.z = random()*0.45+0.75
                #if obj_object.location.z < 0.9:
                #    obj_object.location.z = random()*0.2-0.1
                #    obj_object.location.z = random()*0.2-0.1
        ...
        ambient_light = bpy.data.objects['Area']
        ambient_light.hide_render = True
        projector = bpy.data.objects['Spot']
        ...
        for ob in scene.objects:
            if ob.type == 'CAMERA':          
                if ob.name=='cam_L': #ob.name =='mask':
                    ...
                    auto_file = os.path.join(sample_dir, ob.name+'0020.png')  
                    node= nodes['maskout']
                    ...
                    auto_file_depth = os.path.join(sample_dir+'/temp/', ob.name+'0020.exr')
                    node= nodes['depthout']
                    node.file_slots[0].path = ob.name
                    ...
                    #auto_file_part = os.path.join(sample_dir+'/temp/', ob.name+'0020.png')
                    #node= nodes['part_out']
                    #node.file_slots[0].path = ob.name
                    #node_mix = nodes['Render Layers']
                    ...
                    #auto_file_rgb = os.path.join(sample_dir+'/rgb_temp/', ob.name+'0061.png')
                    #node= nodes['rgbimage']
                    #node.file_slots[0].path = ob.name
                    #node_mix = nodes['Render Layers']
                    #link_rgb = tree.links.new(node_mix.outputs["Diffuse Color"], node.inputs[0])
                    #node.base_path=sample_dir+'/rgb_temp/'
        ...
        #rendred_break = np.multiply(fit, nonearr)
        ...
        lamp_types = ['POINT', 'SPOT']
        lamp_type = Rchoice(lamp_types)
        print('lamp_type: ', lamp_type)
        lamp_name = 'lighting'
        print('lamp_name: ', lamp_name)
        lamp_position = ((random()*2.9-1.45), (random()*2.9-1.45), (random()*2.0+1.0))
        ...
        lamp_object.data.node_tree.nodes['Emission'].inputs['Strength'].default_value = 10.0 + random()*250.0
        lamp_object.location = lamp_position
        lamp_object.rotation_euler.x=radians(random()*120.0-60.0) #0~90
        lamp_object.rotation_euler.y=radians(random()*120.0-60.0) #-45-45
        lamp_object.rotation_euler.z=radians(random()*360.0)
        lamp_object.select = True
        scene.objects.active = lamp_object
        
        ambient_light.data.node_tree.nodes['Emission'].inputs['Strength'].default_value = 50.0 + random()*250.0
        ...
        for ob in scene.objects:
            if ob.type == 'CAMERA':          
                if ob.name=='cam_L': #ob.name =='mask':
                    #Render IR image and Mask
                    bpy.context.scene.camera = ob
                    print('Set camera %s for Color' % ob.name )
                    file_L = os.path.join(sample_dir , ob.name )
                    
                    auto_file_rgb = os.path.join(sample_dir+'/rgb_temp/', ob.name+'0020.png')
                    node= nodes['rgbout']
                    node.file_slots[0].path = ob.name
                    node_mix = nodes['Render Layers']
        ...
        bpy.ops.object.select_all(action='DESELECT')
        bpy.data.objects['lighting'].select = True    # Blender 2.7x
        bpy.ops.object.delete()
